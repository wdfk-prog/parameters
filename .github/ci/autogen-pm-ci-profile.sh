...
#
# Keep this file and autogen-pm-ci-symbols.sh as the single source of truth
# for RT-Thread package compile configuration and static-analysis preprocessor symbols.

autogen_pm_ci_profile_dir="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=.github/ci/autogen-pm-ci-symbols.sh
. "$autogen_pm_ci_profile_dir/autogen-pm-ci-symbols.sh"
