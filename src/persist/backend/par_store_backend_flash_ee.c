 ...
 */
#include "persist/backend/par_store_backend_flash_ee.h"
...
static par_status_t par_store_flash_ee_init(void)
{
    ...
    status = g_par_store_flash_ee_ctx.p_port_api->is_init(g_par_store_flash_ee_ctx.p_port_ctx, &port_is_init);
    if ((ePAR_OK != status) || (false == port_is_init))
    {
        par_store_flash_ee_set_diag(ePAR_STORE_FLASH_EE_DIAG_PORT_INIT);
 ...
 * @return ePAR_OK on success, otherwise an error code.
 */
static par_status_t par_store_flash_ee_is_init(bool *p_is_init)
{
    if (NULL == p_is_init)
    {
        return ePAR_ERROR_PARAM;
