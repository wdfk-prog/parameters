/**
 * @file par_test_nvm_grouped_payload_only.c
 * @brief Provide layout-aware manual MSH helpers for grouped-payload-only scalar NVM validation.
 *
...
/**
 * @brief CRC byte offset inside one serialized grouped-payload-only record.
 */
#define T_RECORD_CRC_OFFSET (0U)

/**
 * @brief Payload start offset inside one serialized grouped-payload-only record.
 */
#define T_RECORD_PAYLOAD_OFFSET (1U)

/**
 * @brief Size descriptor offset inside one serialized grouped-payload-only record.
 */
#define T_RECORD_SIZE_OFFSET (0U)

/**
 * @brief Print one line from the grouped-payload-only MSH helper.
 */
...
{
    T_PRINT("usage:\n");
    T_PRINT("  par_nvm_grouped_payload_only info\n");
    T_PRINT("  par_nvm_grouped_payload_only slot <slot>\n");
    T_PRINT("  par_nvm_grouped_payload_only corrupt_slot_crc <slot> [mask]\n");
    T_PRINT("  par_nvm_grouped_payload_only corrupt_payload <slot> <payload_offset> [mask]\n");
}

/**
 * @brief Print the compiled grouped-payload-only NVM layout summary.
...
static int t_cmd_info(void)
{
    ...
    T_PRINT("layout=grouped_payload_only\n");
...
/**
 * @brief Print one grouped-payload-only slot address calculation.
...
/**
 * @brief Corrupt the CRC byte of one grouped-payload-only slot.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return Shell command status.
 */
static int t_cmd_corrupt_slot_crc(const int argc, char **argv)
{
    ...
    if ((argc < 3) || (false == t_parse_u32(argv[2], &slot)) || ((argc > 3) && (false == t_parse_u32(argv[3], &mask))) ||
        (mask > 0xFFU))
    {
        T_PRINT("ERR usage: par_nvm_grouped_payload_only corrupt_slot_crc <slot> [mask]\n");
        return -1;
    }
    if (false == t_get_slot_info(slot, &info))
    {
...
/**
 * @brief Corrupt one serialized payload byte of a grouped-payload-only slot.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return Shell command status.
 */
static int t_cmd_corrupt_payload(const int argc, char **argv)
{
    ...
    {
        T_PRINT("ERR usage: par_nvm_grouped_payload_only corrupt_payload <slot> <payload_offset> [mask]\n");
        return -1;
    }
    if (false == t_get_slot_info(slot, &info))
    {
...
/**
 * @brief MSH entry point for grouped-payload-only NVM manual tests.
 * @param argc Argument count.
 * @param argv Argument vector.
...
#if defined(RT_USING_FINSH)
/**
 * @brief MSH wrapper for grouped-payload-only NVM manual tests.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return 0 on success; otherwise a negative error code.
 */
static int par_test_nvm_grouped_payload_only_msh(int argc, char **argv)
{
    par_test_bind_rt_console();
    return par_test_nvm_grouped_payload_only_exec(argc, argv);
}

/**
 * @brief Option-completion IDs for par_nvm_grouped_payload_only subcommands.
 */
typedef enum
{
...
CMD_OPTIONS_NODE_START(par_test_nvm_grouped_payload_only_msh)
...
MSH_CMD_EXPORT_ALIAS(par_test_nvm_grouped_payload_only_msh,
                     par_nvm_grouped_payload_only,
                     grouped-payload-only NVM manual tests,
                     optenable);
#endif /* defined(RT_USING_FINSH) */
