/**
 * @file par_test_nvm_compact_payload.c
 * @brief Provide layout-aware manual MSH helpers for compact-payload scalar NVM validation.
 *
...
#if (PAR_CFG_NVM_RECORD_LAYOUT == PAR_CFG_NVM_RECORD_LAYOUT_COMPACT_PAYLOAD)
...
/**
 * @brief CRC byte offset inside one serialized compact-payload record.
 */
#define T_RECORD_CRC_OFFSET (3U)

/**
 * @brief Payload start offset inside one serialized compact-payload record.
 */
#define T_RECORD_PAYLOAD_OFFSET (4U)

/**
 * @brief Size descriptor offset inside one serialized compact-payload record.
 */
#define T_RECORD_SIZE_OFFSET (2U)

/**
 * @brief Print one line from the compact-payload MSH helper.
 */
...
{
    T_PRINT("usage:\n");
    T_PRINT("  par_nvm_compact_payload info\n");
    T_PRINT("  par_nvm_compact_payload slot <slot>\n");
    T_PRINT("  par_nvm_compact_payload corrupt_slot_crc <slot> [mask]\n");
    T_PRINT("  par_nvm_compact_payload corrupt_payload <slot> <payload_offset> [mask]\n");
    T_PRINT("  par_nvm_compact_payload corrupt_size <slot> [mask]\n");
}

/**
 * @brief Print the compiled compact-payload NVM layout summary.
...
static int t_cmd_info(void)
{
    ...
    T_PRINT("layout=compact_payload\n");
...
/**
 * @brief Print one compact-payload slot address calculation.
...
/**
 * @brief Corrupt the CRC byte of one compact-payload slot.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return Shell command status.
 */
static int t_cmd_corrupt_slot_crc(const int argc, char **argv)
{
    ...
    if ((argc < 3) || (false == t_parse_u32(argv[2], &slot)) || ((argc > 3) && (false == t_parse_u32(argv[3], &mask))) ||
        (mask > 0xFFU))
    {
        T_PRINT("ERR usage: par_nvm_compact_payload corrupt_slot_crc <slot> [mask]\n");
        return -1;
    }
    if (false == t_get_slot_info(slot, &info))
    {
...
/**
 * @brief Corrupt one serialized payload byte of a compact-payload slot.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return Shell command status.
 */
static int t_cmd_corrupt_payload(const int argc, char **argv)
{
    ...
    {
        T_PRINT("ERR usage: par_nvm_compact_payload corrupt_payload <slot> <payload_offset> [mask]\n");
        return -1;
    }
    if (false == t_get_slot_info(slot, &info))
    {
...
/**
 * @brief Corrupt the serialized size descriptor of one compact-payload slot.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return Shell command status.
 */
static int t_cmd_corrupt_size(const int argc, char **argv)
{
    uint32_t slot = 0U;
    uint32_t mask = 0x01U;
    uint8_t old_value = 0U;
    ...
    if ((argc < 3) || (false == t_parse_u32(argv[2], &slot)) || ((argc > 3) && (false == t_parse_u32(argv[3], &mask))) ||
        (mask > 0xFFU))
    {
        T_PRINT("ERR usage: par_nvm_compact_payload corrupt_size <slot> [mask]\n");
        return -1;
    }
    if (false == t_get_slot_info(slot, &info))
    {
    ...
    status = t_flip_u8(info.record_offset + T_RECORD_SIZE_OFFSET, (uint8_t)mask, &old_value, &new_value);
    T_PRINT("corrupt_size slot=%lu offset=0x%08lX old=0x%02X new=0x%02X status=%s(0x%04X)\n",
            (unsigned long)slot,
            (unsigned long)(info.record_offset + T_RECORD_SIZE_OFFSET),
            (unsigned)old_value,
            (unsigned)new_value,
            t_status_str(status),
            (unsigned)status);
    T_PRINT("manual next step: reboot and check size validation or rebuild logs\n");
    return t_status_ok(status) ? 0 : -1;
}

/**
 * @brief MSH entry point for compact-payload NVM manual tests.
 * @param argc Argument count.
 * @param argv Argument vector.
...
int par_test_nvm_compact_payload_exec(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "corrupt_size"))
    {
        return t_cmd_corrupt_size(argc, argv);
    }
    t_print_usage();
    return -1;
}
...
#if defined(RT_USING_FINSH)
/**
 * @brief MSH wrapper for compact-payload NVM manual tests.
 * @param argc Argument count.
 * @param argv Argument vector.
 * @return 0 on success; otherwise a negative error code.
 */
static int par_test_nvm_compact_payload_msh(int argc, char **argv)
{
    par_test_bind_rt_console();
    return par_test_nvm_compact_payload_exec(argc, argv);
}

/**
 * @brief Option-completion IDs for par_nvm_compact_payload subcommands.
 */
typedef enum
{
    ...
    T_OPT_CORRUPT_SIZE,    /**< Corrupt one serialized size descriptor. */
} t_opt_id_t;

CMD_OPTIONS_NODE_START(par_test_nvm_compact_payload_msh)
...
CMD_OPTIONS_NODE(T_OPT_CORRUPT_SIZE, corrupt_size, corrupt one serialized size descriptor)
CMD_OPTIONS_NODE_END
MSH_CMD_EXPORT_ALIAS(par_test_nvm_compact_payload_msh, par_nvm_compact_payload, compact-payload NVM manual tests, optenable);
#endif /* defined(RT_USING_FINSH) */

#endif /* (PAR_CFG_NVM_RECORD_LAYOUT == PAR_CFG_NVM_RECORD_LAYOUT_COMPACT_PAYLOAD) */
