[中文](./nvm-fixed-payload-only-manual-test.zh-CN.md)

# Fixed-payload-only scalar NVM manual test

## Scope

This document covers destructive board-level validation for `src/nvm/scalar/layout/par_nvm_layout_fixed_payload_only.c`. The test entry is the dedicated `par_nvm_fixed_payload_only` MSH helper implemented in `parameters/tests/par_test_nvm_fixed_payload_only.c`.
...
Each scalar record stores CRC plus a fixed 4-byte payload slot; ID and size are not serialized.
...
| record size | `1 + 4 = 5 bytes` |
| slot offset rule | `layout addr_from_persist_idx() uses persisted order and fixed 5-byte records` |
...
AUTOGEN_PM_NVM_RECORD_LAYOUT_FIXED_PAYLOAD_ONLY=y
AUTOGEN_PM_TEST_NVM_FIXED_PAYLOAD_ONLY=y
```
...
```sh
par info
par_nvm_fixed_payload_only info
```

Expected `info` checks:

1. `layout=fixed_payload_only`.
...
```sh
par_nvm_fixed_payload_only info
par_nvm_fixed_payload_only slot <slot>
par_nvm_fixed_payload_only corrupt_slot_crc <slot> [mask]
par_nvm_fixed_payload_only corrupt_payload <slot> <payload_offset> [mask]
...
```sh
par_nvm_fixed_payload_only corrupt_slot_crc <slot>
# reboot immediately
```
...
```sh
par_nvm_fixed_payload_only corrupt_payload <slot> <payload_offset>
# reboot immediately
```
...
| --- | --- | --- |
| Command is not listed in MSH completion | Helper Kconfig is disabled, `RT_USING_FINSH` is disabled, or `SConscript` did not include the helper source. | Recheck `AUTOGEN_PM_TEST_NVM_FIXED_PAYLOAD_ONLY`, `AUTOGEN_PM_NVM_RECORD_LAYOUT_FIXED_PAYLOAD_ONLY`, `AUTOGEN_PM_USING_TESTS`, and `RT_USING_FINSH`; rebuild from a clean tree. |
...
- Keep `AUTOGEN_PM_USING_TABLE_ID_CHECK=y`; this layout has no per-record ID and relies on table-ID compatibility.
