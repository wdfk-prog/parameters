[English](./nvm-compact-payload-manual-test.md)

# Compact-payload scalar NVM 手动测试

## 范围

本文档覆盖 `src/nvm/scalar/layout/par_nvm_layout_compact_payload.c` 的破坏性板级验收。测试入口是 `parameters/tests/par_test_nvm_compact_payload.c` 提供的独立 `par_nvm_compact_payload` MSH helper。

该 helper 用于验证 layout-aware 地址计算，并向当前 scalar NVM backend 注入受控损坏。它不是生产命令，只能在 NVM 数据可丢弃的测试固件中启用。

## Layout 模型

每条 scalar record 存储 ID、payload size、CRC，并只保存该参数的自然 payload 字节数。
...
| `corrupt_size <slot> [mask]` | 异或损坏序列化 payload-size 字节，仅 compact-payload 支持。 |
...
6. 在可丢弃 image 上执行 `par_nvm_compact_payload corrupt_size <slot>`，重启后确认 size mismatch 被拒绝，并从默认值重建 scalar image。
...
| 现象 | 可能原因 | 处理 |
| --- | --- | --- |
| MSH 补全中没有该命令 | helper Kconfig 未启用、`RT_USING_FINSH` 未启用，或 `SConscript` 没有纳入源文件。 | 重新检查 `AUTOGEN_PM_TEST_NVM_COMPACT_PAYLOAD`、`AUTOGEN_PM_NVM_RECORD_LAYOUT_COMPACT_PAYLOAD`、`AUTOGEN_PM_USING_TESTS` 和 `RT_USING_FINSH`，并 clean rebuild。 |
...
- size 损坏是该 layout 的专有路径；fixed-payload 与 grouped-payload layout 不携带 size descriptor。
