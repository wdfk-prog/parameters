[English](./nvm-grouped-payload-only-manual-test.md)

# Grouped-payload-only scalar NVM 手动测试

## 范围

本文档覆盖 `src/nvm/scalar/layout/par_nvm_layout_grouped_payload_only.c` 的破坏性板级验收。测试入口是 `parameters/tests/par_test_nvm_grouped_payload_only.c` 提供的独立 `par_nvm_grouped_payload_only` MSH helper。

该 helper 用于验证 layout-aware 地址计算，并向当前 scalar NVM backend 注入受控损坏。它不是生产命令，只能在 NVM 数据可丢弃的测试固件中启用。

## Layout 模型

record 存储 CRC 和自然 payload 字节，并按 8-bit、16-bit、32-bit payload band 分组排列。
...
| 现象 | 可能原因 | 处理 |
| --- | --- | --- |
| MSH 补全中没有该命令 | helper Kconfig 未启用、`RT_USING_FINSH` 未启用，或 `SConscript` 没有纳入源文件。 | 重新检查 `AUTOGEN_PM_TEST_NVM_GROUPED_PAYLOAD_ONLY`、`AUTOGEN_PM_NVM_RECORD_LAYOUT_GROUPED_PAYLOAD_ONLY`、`AUTOGEN_PM_USING_TESTS` 和 `RT_USING_FINSH`，并 clean rebuild。 |
...
- 不要按 `slot * record_size` 手算地址；该 layout 会按 payload band 重排，必须以 helper 输出为准。
