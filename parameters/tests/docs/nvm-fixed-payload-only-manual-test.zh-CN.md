[English](./nvm-fixed-payload-only-manual-test.md)

# Fixed-payload-only scalar NVM 手动测试

## 范围

本文档覆盖 `src/nvm/scalar/layout/par_nvm_layout_fixed_payload_only.c` 的破坏性板级验收。测试入口是 `parameters/tests/par_test_nvm_fixed_payload_only.c` 提供的独立 `par_nvm_fixed_payload_only` MSH helper。

该 helper 用于验证 layout-aware 地址计算，并向当前 scalar NVM backend 注入受控损坏。它不是生产命令，只能在 NVM 数据可丢弃的测试固件中启用。

## Layout 模型

每条 scalar record 存储 CRC 和固定 4 字节 payload slot；不序列化 ID 和 size。
...
| 现象 | 可能原因 | 处理 |
| --- | --- | --- |
| MSH 补全中没有该命令 | helper Kconfig 未启用、`RT_USING_FINSH` 未启用，或 `SConscript` 没有纳入源文件。 | 重新检查 `AUTOGEN_PM_TEST_NVM_FIXED_PAYLOAD_ONLY`、`AUTOGEN_PM_NVM_RECORD_LAYOUT_FIXED_PAYLOAD_ONLY`、`AUTOGEN_PM_USING_TESTS` 和 `RT_USING_FINSH`，并 clean rebuild。 |
...
- 保持 `AUTOGEN_PM_USING_TABLE_ID_CHECK=y`；该 layout 没有 per-record ID，依赖 table-ID 兼容性保护。
