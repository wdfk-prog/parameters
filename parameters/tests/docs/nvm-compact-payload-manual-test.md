[中文](./nvm-compact-payload-manual-test.zh-CN.md)

# Compact-payload scalar NVM manual test

## Scope

This document covers destructive board-level validation for `src/nvm/scalar/layout/par_nvm_layout_compact_payload.c`. The test entry is the dedicated `par_nvm_compact_payload` MSH helper implemented in `parameters/tests/par_test_nvm_compact_payload.c`.
...
Each scalar record stores ID, payload size, CRC, and only the natural payload bytes.
...
| record size offset | `record_offset + 2` |
| record crc offset | `record_offset + 3` |
| record payload offset | `record_offset + 4` |
| record size | `4 + payload_size` |
| slot offset rule | `layout addr_from_persist_idx() prefix-sums natural record sizes` |
...
AUTOGEN_PM_NVM_RECORD_LAYOUT_COMPACT_PAYLOAD=y
AUTOGEN_PM_TEST_NVM_COMPACT_PAYLOAD=y
```
...
```sh
par info
par_nvm_compact_payload info
```

Expected `info` checks:

1. `layout=compact_payload`.
...
```sh
par_nvm_compact_payload info
par_nvm_compact_payload slot <slot>
par_nvm_compact_payload corrupt_slot_crc <slot> [mask]
par_nvm_compact_payload corrupt_payload <slot> <payload_offset> [mask]
par_nvm_compact_payload corrupt_size <slot> [mask]
```
...
| `corrupt_size <slot> [mask]` | XOR the serialized payload-size byte. Compact-payload only. |
...
```sh
par_nvm_compact_payload corrupt_slot_crc <slot>
# reboot immediately
```
...
```sh
par_nvm_compact_payload corrupt_payload <slot> <payload_offset>
# reboot immediately
```
...
6. Run `par_nvm_compact_payload corrupt_size <slot>` on a disposable image, reboot, and confirm that size mismatch is rejected and the scalar image is rebuilt from defaults.
...
| --- | --- | --- |
| Command is not listed in MSH completion | Helper Kconfig is disabled, `RT_USING_FINSH` is disabled, or `SConscript` did not include the helper source. | Recheck `AUTOGEN_PM_TEST_NVM_COMPACT_PAYLOAD`, `AUTOGEN_PM_NVM_RECORD_LAYOUT_COMPACT_PAYLOAD`, `AUTOGEN_PM_USING_TESTS`, and `RT_USING_FINSH`; rebuild from a clean tree. |
...
- Size corruption is a layout-specific path; fixed-payload and grouped-payload layouts do not carry a size descriptor.
