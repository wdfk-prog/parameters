[中文](./nvm-grouped-payload-only-manual-test.zh-CN.md)

# Grouped-payload-only scalar NVM manual test

## Scope

This document covers destructive board-level validation for `src/nvm/scalar/layout/par_nvm_layout_grouped_payload_only.c`. The test entry is the dedicated `par_nvm_grouped_payload_only` MSH helper implemented in `parameters/tests/par_test_nvm_grouped_payload_only.c`.
...
Records store CRC plus natural payload bytes and are grouped by 8-bit, 16-bit, and 32-bit payload bands.
...
| Field | Formula / rule |
| --- | --- |
| record crc offset | `record_offset + 0` |
| record payload offset | `record_offset + 1` |
| record size | `1 + payload_size` |
| slot offset rule | `layout addr_from_persist_idx() reorders slots into 8-bit, 16-bit, and 32-bit bands` |
| schema guard | `requires table-ID checking because records do not carry IDs` |
...
AUTOGEN_PM_NVM_RECORD_LAYOUT_GROUPED_PAYLOAD_ONLY=y
AUTOGEN_PM_TEST_NVM_GROUPED_PAYLOAD_ONLY=y
```
...
```sh
par info
par_nvm_grouped_payload_only info
```

Expected `info` checks:

1. `layout=grouped_payload_only`.
...
```sh
par_nvm_grouped_payload_only info
par_nvm_grouped_payload_only slot <slot>
par_nvm_grouped_payload_only corrupt_slot_crc <slot> [mask]
par_nvm_grouped_payload_only corrupt_payload <slot> <payload_offset> [mask]
...
```sh
par_nvm_grouped_payload_only corrupt_slot_crc <slot>
# reboot immediately
```
...
```sh
par_nvm_grouped_payload_only corrupt_payload <slot> <payload_offset>
# reboot immediately
```
...
| --- | --- | --- |
| Command is not listed in MSH completion | Helper Kconfig is disabled, `RT_USING_FINSH` is disabled, or `SConscript` did not include the helper source. | Recheck `AUTOGEN_PM_TEST_NVM_GROUPED_PAYLOAD_ONLY`, `AUTOGEN_PM_NVM_RECORD_LAYOUT_GROUPED_PAYLOAD_ONLY`, `AUTOGEN_PM_USING_TESTS`, and `RT_USING_FINSH`; rebuild from a clean tree. |
...
- Do not assume `slot * record_size`; always use the helper output because grouped-band ordering can move records relative to persistent order.
