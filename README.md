...
- The persisted `id` is the primary self-description field, so startup and rewrite logic do not rely only on hard-coded slot meaning. The current `size` field is still always written as `4`, so it is mainly a descriptor/integrity helper rather than a space-saving variable-width encoding.
