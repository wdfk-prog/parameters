...
- When `PAR_CFG_TABLE_ID_CHECK_EN = 1`, startup uses a fixed 32-bit FNV-1a table-ID to validate persisted-image compatibility. The table-ID covers `PAR_CFG_TABLE_ID_SCHEMA_VER`, persistent-parameter count, persistent-parameter order, parameter type, and parameter ID. It intentionally does not cover defaults, ranges, names, units, descriptions, or access flags.
