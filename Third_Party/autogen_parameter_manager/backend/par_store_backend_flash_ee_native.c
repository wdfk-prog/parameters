...
#include "../parameters/src/persist/backend/par_store_backend_flash_ee.h"
 ...
 * @return ePAR_OK on success, otherwise an error code.
 */
static par_status_t par_store_flash_ee_native_is_init(const void *p_ctx, bool *p_is_init)
{
    const par_store_flash_ee_native_ctx_t *p_native_ctx = (const par_store_flash_ee_native_ctx_t *)p_ctx;

    if ((NULL == p_native_ctx) || (NULL == p_is_init))
    {
        return ePAR_ERROR_PARAM;
    }
